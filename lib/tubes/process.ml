...
let process prog args =
  ...
  Tube.tube pinfo read read_error write
let%expect_test "read" =
  let module Tube = (val process "echo" ["Hello,"; "World!"]) in
...
let%expect_test "write" =
  let module Tube = (val process "cat" []) in
...
let%expect_test "read_error" =
  let module Tube = (val process "cat" ["definitely_not_a_file"]) in
